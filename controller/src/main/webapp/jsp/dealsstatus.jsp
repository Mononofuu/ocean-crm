...
<fmt:bundle basename="app">
    <head>
        <title><fmt:message key="dealphaseedit"/></title>
    </head>
    <body>
    <div class="col-lg-2">
    ...
    <div class="col-lg-10">
        <div class="row">
            <p class="h1"><fmt:message key="dealphaseedit"/></p>
        </div>
        <table class="table table-hover">
            <c:forEach items="${statusList}" var="entry">
                <tr>
                    ...
                    <td><a href="deal_status?action=edit&id=${entry.id}"><fmt:message key="edit"/></a></td>
                    <c:if test="${!entry.systemDefault}">
                        <td><a href="deal_status?action=delete&id=${entry.id}">Delete<fmt:message key="delete"/></a></td>
                    </c:if>
                </tr>
            </c:forEach>
            <form action="deal_status?action=create" method="post">
                <tr>
                    <td><input name="name" class="text-input" type="text" value=<fmt:message key="dealphasename"/>></td>
                    <td>
                    ...
                    <td>
                        <button class="btn btn-default" type="submit"><fmt:message key="adddealphase"/></button>
                    </td>
                </tr>
            </form>
        </table>
